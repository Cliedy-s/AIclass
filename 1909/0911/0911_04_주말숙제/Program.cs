...
namespace _0911_04_주말숙제
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            RandomNumberGame02 rng = new RandomNumberGame02(); // 랜덤게임 객체
    ...
    class RandomNumberGame02
    {
        ...
        {
            randomArray = new int[ArrLength];
            for (int i = 0; i < randomArray.Length; i++) // 0 - x, 1 - 0, 2 - 1, 0 ...  비교
            {
                randomArray[i] = rand.Next(0, 9); // i번째 입력 후 이전값들과 비교
                for (int j = i - 1; j >= 0; j--)
                {
                    if (randomArray[i] == randomArray[j]) // 이전 값에 걸릴경우
                    {
                        randomArray[i] = rand.Next(0, 9); // 변경
        ...
        public void CheckStrikesAndBalls(string inputNumbers)
        {
            ...
            for (int i = 0; i < randomArray.Length; i++)
            {
                ...
                {
                    Debug.WriteLine("Alambana.RandomNumberGame :: randomArray[{0}]{1} <-> charArr[{2}]{3} ", i, randomArray[i], j, intArr[i]);
                    if (randomArray[i] == intArr[j] && i == j)
                    {
                        Strikes++;
                        break;
                    }
                    else if (randomArray[i] == intArr[j])
                    {
                        Balls++;
                        break;
                    }
