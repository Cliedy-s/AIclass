    ...
    public partial class UserIDPWDSearch : Form
    {
        ...
        private void BtnIdsearch_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("이메일이나 주민번호를 다시확인하여 주십시오");
        ...
        private void BtnPWDsearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                UserInfoVO uvo = new UserInfoVO();
                uvo.cus_Name = txtPWDSearchID.Text.Trim();
                ...
                {
                    MessageBox.Show("아이디와 이메일 주민등록번호를. 다시확인하여 주십시오");
                    return;
                }
                else
                {
                    ...
                    {
                        this.Cursor = Cursors.WaitCursor;
                        string newPwd = PwdSendMail(newpwd, txtPWDSearchEamil.Text.Trim(), uvo);
        ...
        string PwdSendMail(string newPwd, string email , UserInfoVO uvo)
        {
            while (true)
            {
                try
                {
                    ...
                    message.Subject = string.Format("{0}님의 비밀번호 초기화 안내 메일입니다.", uvo.cus_Name);
